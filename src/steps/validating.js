import warningImage from '../img/warning-icon.svg'
import utilsService from '../services/utils.service'

function createValidating() {
  const stepValidating = utilsService.createElementWithClass('div', 'modal-body-wrapper hide')
  stepValidating.id = 'step-VALIDATING'

  stepValidating.innerHTML = `
    <p class="subtitle font-bold">¡En validación!</p>
    <p class="subtitle">Estamos procesamos tu pago. Realizaremos una validación y te confirmaremos en la brevedad.</p>
    <img src="${warningImage}" alt="logo" class="mb-32" />
  ...
  const continueButton = stepValidating.querySelector('#btn-validating-continue')
